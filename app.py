...
def get_random_user_agent():
    return requests.get('https://raw.githubusercontent.com/tamimibrahim17/List-of-user-agents/master/Chrome.txt').text.split('\n')[random.randint(0, 132)]
...
def get_available_formats(url, max_retries=5, retry_delay=10):
    proxies = [None]  # Add your proxy list here if available
    ...
    for attempt in range(max_retries):
        for proxy in proxies:
            try:
                ...
                st.warning(f"Attempt {attempt + 1} with proxy {proxy} failed: {str(e)}")
        ...
        for attempt in range(max_retries):
            for proxy in proxies:
                try:
                    ...
                    st.warning(f"Download attempt {attempt + 1} with proxy {proxy} failed: {str(e)}")
                    if attempt < max_retries - 1:
                        st.info(f"Retrying in {retry_delay} seconds...")
                        time.sleep(retry_delay)
                    else:
